...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <?php if ($feedback): ?>
                    <?php foreach ($feedback as $pesan): ?>
                        <!-- <li>
                            <?= htmlspecialchars($pesan->pesan); ?>
                            <em>(<?= date('d M Y, H:i', strtotime($pesan->created_at)); ?>)</em>
                        </li> -->
                        <h2>Pengaduan ID: <?= htmlspecialchars($pesan->id_pengaduan); ?></h2>
                        <h2><strong>Tanggal:</strong> <?= htmlspecialchars($pesan->created_at); ?></h2>
                        <h2><strong>Pesan Dari Admin: </strong><?= nl2br(htmlspecialchars($pesan->pesan)); ?></h2>
                    <?php endforeach; ?>
                <?php else: ?>
